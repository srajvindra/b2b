...
import { ArrowLeft, ChevronRight, Search, Settings, Users, Plus, Pencil } from "lucide-react"
...
function RolesPermissionsPage() {
  ...
  const [rolesVisibleCount, setRolesVisibleCount] = useState(10)
  const [categoriesVisibleCount, setCategoriesVisibleCount] = useState(10)
  ...
  const paginatedCategories = filteredCategories.slice(0, categoriesVisibleCount)

  useEffect(() => {
    setCategoriesVisibleCount(10)
  }, [permissionSearch])
    ...
    return (
      <div className="p-6">
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b align-top">
                          <th className="text-left font-semibold text-sm text-foreground py-2 pr-4 whitespace-normal break-words">
                            Permission
                          </th>
        ...
        <LoadMorePagination
          ...
          visibleCount={categoriesVisibleCount}
          label="permission categories"
          onLoadMore={() => setCategoriesVisibleCount((prev) => Math.min(prev + 10, filteredCategories.length))}
        />
      </div>
    )
  }

  const paginatedRoles = roles.slice(0, rolesVisibleCount)
  ...
  return (
    <div className="p-6">
      ...
      <LoadMorePagination
        total={roles.length}
        visibleCount={rolesVisibleCount}
        label="roles"
        onLoadMore={() => setRolesVisibleCount((prev) => Math.min(prev + 10, roles.length))}
      />
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (!name) return
                const maxId = roles.reduce((max, r) => Math.max(max, Number.parseInt(r.id, 10) || 0), 0)
                const id = String(maxId + 1)
                setRoles((prev) => [{ id, name, description, usersCount: 0 }, ...prev])
                setRolesVisibleCount((prev) => Math.max(prev, 10))
      ...
      >
        <DialogContent>
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button
              className="bg-teal-600 hover:bg-teal-700 text-white"
              disabled={!editRoleName.trim() || !editingRoleId}
              onClick={() => {
                ...
                if (!name) return
                setRoles((prev) =>
                  prev.map((r) => (r.id === editingRoleId ? { ...r, name, description } : r)),
                )
