...
import { Plus, X, Trash2, Mail, Search, Eye, Pencil, AlertTriangle, Send, Code, Bold, Italic, Underline, Strikethrough, AlignLeft, List, ListOrdered, Link, ImageIcon, Smartphone, Video, Printer, Copy } from "lucide-react"
...
export default function EmailTemplatePage() {
  ...
  const [newTemplate, setNewTemplate] = useState<Omit<EmailTemplate, "id" | "createdOn">>({
    name: "", subject: "", content: "", createdBy: { name: "Nina Patel", role: "Admin" },
    files: 0, sends: 0, opened: 0, clicked: 0,
  })
  ...
  const handleCreate = () => {
    ...
    setTemplates((prev) => [...prev, {
      ...newTemplate, id: `e-${Date.now()}`, createdOn: new Date().toISOString().slice(0, 10),
      files: 0, sends: 0, opened: 0, clicked: 0,
    }])
    setNewTemplate({ name: "", subject: "", content: "", createdBy: { name: "Nina Patel", role: "Admin" }, files: 0, sends: 0, opened: 0, clicked: 0 })
    setIsCreating(false)
  }
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Create and manage email templates for your workflows.</p>
        </div>
          ...
          <tbody className="divide-y divide-border">
            {filteredTemplates.map((t) => (
              <tr key={t.id} className="hover:bg-muted transition-colors">
                ...
                <td className="px-4 py-4 text-sm text-muted-foreground truncate max-w-[200px]">{t.subject}</td>
                <td className="px-4 py-4">
                ...
                <td className="px-4 py-4 text-center">
                  <div className="text-sm font-semibold text-foreground">{t.files}</div>
                ...
                <td className="px-4 py-4 text-center">
                  <div className="text-sm font-semibold text-foreground">{t.opened} %</div>
                  <div className="text-xs text-muted-foreground">opened</div>
                </td>
                <td className="px-4 py-4 text-center">
                  <div className="text-sm font-semibold text-foreground">{t.clicked} %</div>
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Button variant="ghost" size="sm" className="text-primary hover:text-primary/80"><Send className="h-4 w-4" /></Button>
      ...
      <Dialog open={isCreating} onOpenChange={setIsCreating}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Mail className="h-5 w-5 text-blue-600" /> Create Email Template</DialogTitle>
            <DialogDescription>Fill in the details for your new email template.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input value={newTemplate.subject} onChange={(e) => setNewTemplate({ ...newTemplate, subject: e.target.value })} placeholder="Enter subject..." />
            </div>
            <div className="space-y-2">
              <Label>Content</Label>
              <div className="border border-border rounded-lg focus-within:ring-2 focus-within:ring-primary focus-within:border-transparent overflow-hidden">
                <textarea value={newTemplate.content} onChange={(e) => setNewTemplate({ ...newTemplate, content: e.target.value })} className="w-full min-h-[200px] p-3 text-sm resize-none focus:outline-none border-none" placeholder="Write your email content..." />
      ...
      <Dialog open={!!editingTemplate} onOpenChange={() => setEditingTemplate(null)}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Mail className="h-5 w-5 text-blue-600" /> {isViewMode ? "View Template" : "Edit Template"}</DialogTitle>
          ...
          {editingTemplate && (
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <Label>Subject Line</Label>
                <Input value={editingTemplate.subject} onChange={(e) => setEditingTemplate({ ...editingTemplate, subject: e.target.value })} disabled={isViewMode} className={isViewMode ? "bg-muted" : ""} />
              </div>
              <div className="space-y-2">
                <Label>Content</Label>
                <div className={`border border-border rounded-lg focus-within:ring-2 focus-within:ring-primary focus-within:border-transparent overflow-hidden ${isViewMode ? "bg-muted" : ""}`}>
                  <textarea value={editingTemplate.content} onChange={(e) => setEditingTemplate({ ...editingTemplate, content: e.target.value })} disabled={isViewMode} className={`w-full min-h-[200px] p-3 text-sm resize-none focus:outline-none border-none ${isViewMode ? "bg-muted" : ""}`} />
                  {!isViewMode && (
                    <div className="flex items-center gap-1 px-3 py-2 border-t border-border bg-muted/30">
                      <button type="button" className="p-1.5 rounded hover:bg-accent text-muted-foreground hover:text-foreground transition-colors"><Code className="h-4 w-4" /></button>
                      <button type="button" className="p-1.5 rounded hover:bg-accent text-muted-foreground hover:text-foreground transition-colors"><Bold className="h-4 w-4" /></button>
                ...
                {!isViewMode && <p className="text-xs text-muted-foreground">Use {"{{variable}}"} syntax for dynamic content.</p>}
              </div>
