...
export function Combined({
  ...
  const [commEscalatedOverrides, setCommEscalatedOverrides] = useState<Record<number, string>>({})
  ...
  const [commEscalateSelectedStaff, setCommEscalateSelectedStaff] = useState("")
  const [commEscalateStaffPopoverOpen, setCommEscalateStaffPopoverOpen] = useState(false)
  ...
  const summary = useMemo(() => {
    const tasks = combinedItems.filter((i) => i.kind === "task").map((i) => i.task)
    const comms = combinedItems.filter((i) => i.kind === "comm").map((i) => i.comm)

    const openTasks = tasks.filter((t) => t.status !== "Completed" && t.status !== "Skipped" && !t.overdue).length
  ...
  return (
    <>
            ...
            <div className="flex-1 rounded-lg border border-slate-200 bg-white overflow-hidden">
              <div className="grid grid-cols-2">
                <div className="px-3 py-2">
                  <div className="text-xs font-semibold text-slate-600 mb-1">Tasks</div>
                ...
                <div className="border-l border-slate-200 px-3 py-2">
                  <div className="text-xs font-semibold text-slate-600 mb-1">Communication</div>
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white border border-slate-200">
                      <div className="w-9 h-9 rounded-lg bg-slate-100 flex items-center justify-center">
            ...
            <div className={`max-h-[${maxHeight}] overflow-y-auto`}>
              <Table>
                <TableHeader className="sticky top-0 z-10 bg-slate-50">
                  ...
                  <TableHead className="font-medium text-slate-700 p-0">
                  ...
                  <TableHead className="font-medium text-slate-700 p-0 text-right">
                    <Popover open={dateTimeFilterOpen} onOpenChange={setDateTimeFilterOpen}>
                      <PopoverTrigger asChild>
                        <button
                          type="button"
                          className="w-full flex items-center justify-end gap-1 px-3 py-2 text-right hover:bg-slate-100"
                        >
                          <span>Date/Time</span>
                    ...
                    visibleItems.map((item) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell className="text-sm text-slate-600">
                              <Popover
                                open={assignPopoverOpen === comm.id}
                                onOpenChange={(open) => setAssignPopoverOpen(open ? comm.id : null)}
                              >
                                <PopoverTrigger asChild>
                                  <button
                                    type="button"
                            ...
                            <TableCell className="text-sm text-slate-600">
                              <Popover
                                ...
                                onOpenChange={(open) =>
                                  setEscalatedToPopoverOpen(open ? comm.id : null)
                                }
                              >
                                <PopoverTrigger asChild>
                                  <button
                                  ...
                                  >
                                    {commEscalatedTo ? (
                                      <>
                                        <div className="w-5 h-5 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-medium text-slate-600 shrink-0">
                                          {(() => {
                                            const [firstName, lastName] = commEscalatedTo.split(" ")
                                            return (firstName?.[0] ?? "") + (lastName?.[0] ?? "")
                                          })()}
                                        </div>
                                        <span className="truncate">{commEscalatedTo}</span>
                                      </>
                                    ) : (
                                      <>
                                        ...
                                        {escalatedToStaffMembers.map((staff) => {
                                          ...
                                          return (
                                            <CommandItem
                                              ...
                                              value={staff.name}
                                              onSelect={() => {
                                                onEscalateCommunication?.(comm.id, displayValue)
                                                setCommEscalatedOverrides((prev) => ({
                                                  ...prev,
                                                  [comm.id]: displayValue,
                                                }))
                                ...
                                <DropdownMenuContent align="end" className="w-40">
                                  ...
                                  <DropdownMenuItem
                                    onClick={(e) => {
                                      ...
                                      setCommEscalateSelectedStaff(commEscalatedTo || "")
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell className="text-sm text-slate-600">
                            <span className="block truncate">{task.entity}</span>
                          </TableCell>
                          <TableCell className="text-sm text-slate-600">
                            <Popover
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <Popover open={escalateStaffPopoverOpen} onOpenChange={setEscalateStaffPopoverOpen}>
            <PopoverTrigger asChild>
              <button
                type="button"
                className="w-full flex items-center justify-between px-3 py-2 rounded-md border border-slate-200 bg-white hover:bg-slate-50 text-left"
              >
                {escalateSelectedStaff ? (
                  <span className="text-sm text-slate-800 truncate">{escalateSelectedStaff}</span>
                ) : (
                  <span className="text-sm text-slate-400">Select staff...</span>
                )}
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    {escalatedToStaffMembers.map((staff) => {
                      const displayValue = `${staff.name} (${staff.role})`
                      const isSelected =
                        escalateSelectedStaff === displayValue || escalateSelectedStaff === staff.name
                      return (
                        <CommandItem
                          key={staff.id}
                          value={staff.name}
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button
              ...
              onClick={() => {
                if (!escalateTask || !escalateSelectedStaff) return
      ...
      <Dialog open={commEscalateModalOpen} onOpenChange={setCommEscalateModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <TriangleAlert className="h-5 w-5 text-amber-500" />
            ...
            {escalateComm && (
              <p className="text-sm text-slate-500 mt-1">{escalateComm.from}</p>
            )}
          </DialogHeader>

          <div className="text-sm text-slate-600">
            Select a staff member to escalate this communication to:
          </div>

          <Popover open={commEscalateStaffPopoverOpen} onOpenChange={setCommEscalateStaffPopoverOpen}>
            <PopoverTrigger asChild>
              <button
                type="button"
                className="w-full flex items-center justify-between px-3 py-2 rounded-md border border-slate-200 bg-white hover:bg-slate-50 text-left"
              >
                {commEscalateSelectedStaff ? (
                  <span className="text-sm text-slate-800 truncate">{commEscalateSelectedStaff}</span>
                ) : (
                  <span className="text-sm text-slate-400">Select staff...</span>
                )}
                <ChevronsUpDown className="h-4 w-4 text-slate-400 shrink-0" />
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    {escalatedToStaffMembers.map((staff) => {
                      const displayValue = `${staff.name} (${staff.role})`
                      const isSelected =
                        commEscalateSelectedStaff === displayValue || commEscalateSelectedStaff === staff.name
                      return (
                        <CommandItem
                          key={staff.id}
                          value={staff.name}
                          onSelect={() => {
                            setCommEscalateSelectedStaff(displayValue)
                            setCommEscalateStaffPopoverOpen(false)
                          }}
                          className="flex items-center gap-2"
                        >
                          <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-medium text-slate-600 shrink-0">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            <Button
              ...
              onClick={() => {
                ...
                setEscalateComm(null)
                setCommEscalateSelectedStaff("")
              }}
            >
              Cancel
            </Button>
            <Button
              className="bg-amber-500 hover:bg-amber-600 text-white"
              disabled={!commEscalateSelectedStaff}
              onClick={() => {
                if (!escalateComm || !commEscalateSelectedStaff) return
                onEscalateCommunication?.(escalateComm.id, commEscalateSelectedStaff)
                setCommEscalatedOverrides((prev) => ({ ...prev, [escalateComm.id]: commEscalateSelectedStaff }))
                setCommEscalateModalOpen(false)
              }}
            >
              Escalate
